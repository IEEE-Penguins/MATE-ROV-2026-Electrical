...
ESCChannel M3 = ESCChannel(ESC_3);
...
ServoDriver servo2 = ServoDriver(5, 0);
...
void loop() {
  ...
  // for (int i = 0; i < 5; i++) thrusters_speeds[i] = 0.3;
  ...
  if (Serial.available()) {
    ...
    if (!error) {
      ...
      for (int i = 0; i < 3; i++) servos_directions[i] = servo[i];
